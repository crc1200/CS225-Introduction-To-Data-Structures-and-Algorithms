...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    std::cout << "the child index is " << child_idx << std::endl;
    std::cout << "parent size is " << parent -> children.size() <<std::endl;
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    //If subroot elements at (first larger index).key is NOT equal to pair.key
    // first larger index == size of elements?
