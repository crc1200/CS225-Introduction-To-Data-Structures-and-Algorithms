...
class KDTree
{
  ...
  public:
    ...
    Point<Dim> quickSelect(vector<Point<Dim>>& points, int l, int r, int k, int dim);
    void buildTree(vector<Point<Dim>>& points, int dim, int left, int right, KDTreeNode *& curr);
    void  swap(Point<Dim>* a, Point<Dim>* b);
    Point<Dim> neighborHelper(Point<Dim>& query, int dim, KDTreeNode *& curr) const;
