                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    if (singleQueue.empty() && mergeQueue.empty()) {
        ...
        auto x = mergeQueue.front();
        mergeQueue.pop();
        return x;
    } else if (mergeQueue.empty()) {
        auto x = singleQueue.front();
        singleQueue.pop();
        return x;
    }
    TreeNode * singleQueueNode = singleQueue.front();
    TreeNode * mergeQueueNode = mergeQueue.front();
    if (mergeQueueNode -> freq.getFrequency() < singleQueueNode -> freq.getFrequency()) {
        mergeQueue.pop();
        return mergeQueueNode;
    } else {
        singleQueue.pop();
        return singleQueueNode;
    }
    return NULL;
}
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while (!((singleQueue.size() == 1 && mergeQueue.empty()) || (singleQueue.empty() && mergeQueue.size() == 1))) {
        TreeNode* m1 = removeSmallest(singleQueue, mergeQueue);
        TreeNode* m2 = removeSmallest(singleQueue, mergeQueue);
        Frequency inF(m1 -> freq.getCharacter() + m2 -> freq.getCharacter() , m1 -> freq.getFrequency() + m2 -> freq.getFrequency());
        TreeNode* n = new TreeNode(inF);
        n -> left = m1;
        n -> right = m2;
        mergeQueue.push(n);
    }
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
     ...
     */
     while (bfile.hasBits()) {
        auto c = bfile.getNextBit();
        if (c) {    //we read a 1
            char character = bfile.getNextByte();
            Frequency inF(character, 0);
            return  new TreeNode(inF);
        } else {    //we read a 0
            //char character = bfile.getNextByte();
            //Frequency inF(0);
            auto res = new TreeNode(0);
